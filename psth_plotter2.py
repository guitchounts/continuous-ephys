...
if __name__ == "__main__":
	...
	channels =    np.array([13]) # np.arange(32) # 32 
	...
	np.savez('ephys_all_trials',all_trials=all_trials)
	print '######################  all trials saved.... #######################'

	psth_folder = './psth2/'
	time = np.linspace(-0.5,0.5,30000) # time vec goes from -0.5 sec to +0.5 sec in 30000 steps.
	for ch_idx,ch in enumerate(channels):
		for ori in stim_dict.keys():
			save_folder = psth_folder + 'channel_' + str(ch+1)
			#print stim_key, ' : ', stim_dict[stim_key]
			if not os.path.exists(save_folder):
				os.makedirs(save_folder)
			plot_raw_traces(all_trials[:,stim_dict[str(ori)],ch_idx],time,ori,ch,save_folder) 
