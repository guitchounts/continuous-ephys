...
if __name__ == "__main__":
	...
	tree_after = ET.parse(sys.argv[2])
	...
	b = pd.DataFrame(data={
	    'after': impedances_after,
	    'before': impedances_before})

	ax = sns.boxplot(data=b,order=['before','after'],palette="Set2")
	ax = sns.stripplot(data=b, color=".25",order=['before','after'],alpha=0.25)
	#ax.set_ylim([0,2e7])

	for i in range(len(impedances_before)):
	    plt.plot( [0,1], [impedances_before[i], impedances_after[i]], c='k',alpha=0.25)
	...
	legend1 = ax.legend([before_mean,before_median],loc=2)
