...
from cluster_histogram_plot import spike_trials
...
def get_file_metadata(files):
	...
	for meta in all_metadata:
		print 'start time =', meta['start_time'] #/meta['sample_rate']
		print 'len = ', meta['data_len']
...
def organize_stim_dict(stim_data):
	# input = stim_data
	# output = dictionary of orientations and their indices within the stim_data structure, e.g. '18':[4,10,34,50,54]
...
def plot_raw_traces(traces,time,stim_name,ch,save_folder,stim_dict,stim_data):
	...
	ax = range(num_trials)
	all_spike_times = []
	raster = figure(width=1000, height=400,y_axis_label='Trial Number',title='Raster + Histogram Channel %d, Orientation %s' % (ch+1,stim_name))
	spike_vec = np.zeros([len(time),1])
	
	time_vec = np.linspace(0,len(time),len(time))
	print 'stim_dict[stim_name] = ', stim_dict[stim_name]
	print 'stim_data.times[stim_dict[stim_name]] = ', stim_data.times[stim_dict[stim_name]] 
	print 'stim_name = ', stim_name
	

	spike_mat = get_sua_times(stim_data.times[stim_dict[stim_name]]) ### this will need times of the stimuli...
...
def get_sua_times(stim_times):
	# for a given trial, return samples on which spikes occur, between -15e3 and 15e3 (with 0 = stim_time)
	print 'stim time = ', stim_times

	trial_len = 30e3 / 2 ## !! this is the half-range.

	# get trial times
	
		#spike_mat[trial,spike_times] +=1

	spike_mat = np.zeros([len(stim_times),trial_len*2]) # trials x trial_length

	spikes_file = 'subcluster2.mat'

	spikes = sio.loadmat(spikes_file)
	clust_name = sio.whosmat(spikes_file)[0][0]
	print 'whosmat!! ', spikes[clust_name].shape
	
	experiment_lengths = [meta['data_len'] for meta in metadata]

	spike_times = spike_trials(spikes,clust_name,experiment_lengths)
	
	print 'spike times = ', spike_times

	for stim_time in stim_times:

		trial_min = stim_time - trial_len
		trial_max = stim_time + trial_len
		starts_ind = np.where(spike_times>trial_min)
		stops_ind = np.where(spike_times<trial_max)
		
		start_stop_indices = np.intersect1d(starts_ind,stops_ind)
		print start_stop_indices
		
		spike_mat[spike_times[start_stop_indices]] += 1

	print 'shape of spike_mat before returning from get_sua_times = ', spike_mat.shape
	return spike_mat

def concatenate_kwd(raw_files,channels):
	#ephys = np.array([])
	
	for file_idx,file in enumerate(raw_files):
		kwd_file = h5py.File(file,'r')		
		tmp_data = np.array(kwd_file['/recordings/0/data']) # = samples x channels			
		#for ch in channels:
		#	tmp_data[:,ch] = filter(tmp_data[:,ch],[500, 12e3])
		
		print 'file_idx = ', file_idx

		if file_idx == 0:
			print 'first'
			ephys = tmp_data[:,channels]
		else:
			print 'next'
			print '! ephys shape = ',ephys.shape
			print '! tmp_data[:,channels].shape = ',tmp_data[:,channels].shape
			ephys = np.vstack((ephys,tmp_data[:,channels]))
		print 'ephys temp shape = ', ephys.shape
		tmp_data = None
		...
		#all_metadata.append(metadata)
		kwd_file.close()
	print 'shape of ephys = ', ephys.shape
	return ephys

def get_trial_data(ephys,channels,stim_data):
	trial_range = 15000 # samples... 
	print 'len(stim_data) = ',len(stim_data)
	print 'type len(stim_data) = ',type(len(stim_data))
	all_trials = np.zeros((trial_range*2,len(stim_data),len(channels))) # samples x trials x channels
	erase_trials = []
	for trial,stim_time in enumerate(stim_data):
		tmp_trial = np.zeros((trial_range*2,len(channels)))
		if stim_time+trial_range < len(ephys):
			tmp_trial = ephys[stim_time-trial_range:stim_time+trial_range,:] # all channels...
			print 'tmp_trial shape = ', tmp_trial.shape
			print 'Trial %d at time %d falls within of the length of ephys. Adding.' % (trial,stim_time)
			all_trials[:,trial,:] = tmp_trial
		else:
			print 'Trial %d at time %d falls outside of the length of ephys. Skipping.' % (trial,stim_time)
			erase_trials.append(trial)
	#all_trials = all_trials[~np.all(all_trials==0,axis=1)] ## get rid of trials with all zeros
...
if __name__ == "__main__":
	...
	stim_dict = organize_stim_dict(stim_data)
	print 'stim_dict = ',stim_dict
	

	all_trials = get_trial_data(newephys,channels,stim_data.times) # samples x trials x channels

	psth_folder = './psth2/'
	time = np.linspace(-0.5,0.5,30000) # time vec goes from -0.5 sec to +0.5 sec in 30000 steps.
	for ch_idx,ch in enumerate(channels):
		for ori in stim_dict:
			save_folder = psth_folder + 'channel_' + str(ch+1)
			#print stim_key, ' : ', stim_dict[stim_key]
			if not os.path.exists(save_folder):
				os.makedirs(save_folder)
			print 'stim_dict[ori] = ', stim_dict[ori]
			print 'ori = ', ori
			plot_raw_traces(all_trials[:,stim_dict[ori],ch_idx],time,ori,ch,save_folder,stim_dict,stim_data) 
				
			#plot_sua_raster(spike_times,stim_times,save_folder)
